...
//Utility function for obtaining the answer to a yes or no question
...
bool read_network(ifstream &input, double &offset, vector<vec2d> &points, vector<edge_datum> &edges,double &min_x, double &max_x, double &min_y, double &max_y){
	...
	if(num_read == 3){
		...
		dx = get<0>(points[idx1]) - get<0>(points[idx2]);
		dy = get<1>(points[idx1]) - get<1>(points[idx2]);
...
//impose the specified program of strains, perform structural relaxation,
...
void do_relaxation_run(string dat_name, string log_name, string disp_base, string e_name, double ks, double kb, double mu, double min_c, double max_c, double c_inc, double min_s, double max_s, double s_inc, bool strain_mode, bool affine, cholmod_common *cc){
    ...
    //Arrays to keep track of the displacements, pseudo-velocities and
    //pseudo-forces on the vertices of a network
...
void run_relaxation_batch(string file_name, bool use_gpu, bool strain_mode, bool affine){
...
void run_relaxation_interactive(bool use_gpu, bool strain_mode, bool affine){
...
int main(int argc, char **argv){
    ...
    char c;
    char *batch_file;
    bool batch_mode = false, use_gpu = false, strain_mode = false,affine = true;
    ...
    //usage and exit.
    while((c = getopt(argc, argv, "b:gsz")) != -1){
	switch(c) {
	    case 'b':
	...
	free(batch_file);
	run_relaxation_batch(bstring, use_gpu, strain_mode, affine);
    }
    else{
        run_relaxation_interactive(use_gpu, strain_mode, affine);
    }
